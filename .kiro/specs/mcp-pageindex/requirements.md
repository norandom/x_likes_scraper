...
4. The X Likes MCP Server shall treat `OPENAI_BASE_URL` as an OpenAI-compatible base URL (for example `http://10.0.0.59:8317`) and shall not call any hosted LLM service by default.
5. The X Likes MCP Server shall pass `model=OPENAI_MODEL` and `api_key=OPENAI_API_KEY` to PageIndex's `call_llm` interface, and shall ensure `OPENAI_BASE_URL` is set in the process environment before any PageIndex call so the OpenAI Python SDK that PageIndex constructs internally picks the base URL up at client-construction time.
6. The X Likes MCP Server shall extend `.env.sample` to document `OPENAI_BASE_URL`, `OPENAI_API_KEY`, and `OPENAI_MODEL`, including a note that the endpoint is OpenAI-compatible and local by default.
...
2. The X Likes MCP Server test suite shall mock the LLM call (the function or client method that hits `OPENAI_BASE_URL`) and shall mock the PageIndex tree builder for the unit-test layer.
...
2. The X Likes MCP Server documentation shall list the required `.env` variables (`OPENAI_BASE_URL`, `OPENAI_API_KEY`) and the prerequisite that `scrape.sh` has been run at least once.
...
1. If startup configuration is invalid (missing `OPENAI_BASE_URL`, missing output directory, empty `output/by_month/`), the X Likes MCP Server shall exit non-zero before announcing tools, with a single error line that names the failing condition.
