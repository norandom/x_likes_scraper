...
1. When the X Likes MCP Server starts, the X Likes MCP Server shall read `LLM_API_BASE` and `LLM_API_KEY` from the project's `.env` file (and from the process environment if `.env` is absent).
...
3. If `LLM_API_BASE` is unset or empty at startup, the X Likes MCP Server shall fail loudly with an error message that names the missing variable.
4. The X Likes MCP Server shall treat `LLM_API_BASE` as an OpenAI-compatible base URL (for example `http://localhost:8080/v1`) and shall not call any hosted LLM service by default.
5. The X Likes MCP Server shall extend `.env.sample` to document `LLM_API_BASE` and `LLM_API_KEY`, including a note that the endpoint is OpenAI-compatible and local by default.
...
2. The X Likes MCP Server shall include in each `search_likes` result the tweet ID, the month (`YYYY-MM`), the handle, and a snippet of the tweet text.
3. When an MCP client calls `search_likes` with an empty or whitespace-only query, the X Likes MCP Server shall return an input-validation error rather than calling the LLM.
4. If the underlying LLM call fails (network error, non-2xx response, malformed body), the X Likes MCP Server shall surface a tool error with a message identifying the failure category, and shall not crash the server process.
5. When PageIndex returns no matches for a valid query, the X Likes MCP Server shall return an empty result list rather than an error.
6. The X Likes MCP Server shall declare a JSON schema for `search_likes` input and output that an MCP client can introspect.
...
4. The X Likes MCP Server shall route every LLM call through the configured `LLM_API_BASE`; if `LLM_API_BASE` points at a hosted vendor, that is the user's choice, not the server's default.
...
1. When a developer runs `pytest tests/` from the repository root with no `LLM_API_BASE` set, the X Likes MCP Server test suite shall execute and finish without making real HTTP calls to any LLM endpoint.
2. The X Likes MCP Server test suite shall mock the LLM call (the function or client method that hits `LLM_API_BASE`) and shall mock the PageIndex tree builder for the unit-test layer.
...
2. The X Likes MCP Server documentation shall list the required `.env` variables (`LLM_API_BASE`, `LLM_API_KEY`) and the prerequisite that `scrape.sh` has been run at least once.
...
1. If startup configuration is invalid (missing `LLM_API_BASE`, missing output directory, empty `output/by_month/`), the X Likes MCP Server shall exit non-zero before announcing tools, with a single error line that names the failing condition.
